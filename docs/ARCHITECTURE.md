...
| **Dependencies / supply chain** | No third-party runtime packages. Uses Node's built-in `http` and `node:sqlite`, and vanilla ES modules with no build step. | Some routing and validation code is hand-written. `node:sqlite` is still flagged experimental, so it is pinned to Node ≥ 22.13. |
| **Data** | SQLite in WAL mode, forward-only numbered SQL migrations, money stored as integer cents, every stock change recorded in `stock_movements`. | Postgres was not chosen because a till must keep selling when the WAN is down. A store-local DB avoids that. |
